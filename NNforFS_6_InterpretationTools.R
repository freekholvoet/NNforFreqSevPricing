...
NC_CANN_GBM_flex_SURR_allFolds_FR <- lapply(1:6, function(fold){
  # Tune the surrogate technique and determine best split
  autotune_FH_print(NC_object_FR[[fold]],
...
NC_PDP_plot_ageph_ModelComp_AUS <- rbind(
  ...
  theme_bw() + 
  guides(color = guide_legend(nrow = 1, byrow = TRUE)) + 
  xlab(vars_with_label_AUS %>% filter(Variable == 'DrivAge') %>% pull(xlabels)) + ylab("Partial dependence effect") + 
  theme(legend.position="bottom", legend.direction="horizontal", 
        plot.title = element_text(size=18, margin=margin(0,0,50,0)),
...
NC_PDP_plot_ageph_ModelComp_BE <- rbind(
  ...
  theme_bw() + 
  guides(color = guide_legend(nrow = 1, byrow = TRUE)) + 
  xlab(vars_with_label_BE %>% filter(Variable == 'ageph') %>% pull(xlabels)) + ylab("Partial dependence effect") + 
  theme(legend.position="bottom", legend.direction="horizontal", 
        plot.title = element_text(size=18, margin=margin(0,0,50,0)),
        axis.title=element_text(size=12), plot.title.position = "plot",
        plot.subtitle=element_text(size=18, color="black")) + 
...
NC_PDP_plot_ageph_ModelComp_FR <- rbind(
  ...
  theme_bw() + 
  guides(color = guide_legend(nrow = 1, byrow = TRUE)) + 
  xlab(vars_with_label_FR %>% filter(Variable == 'DrivAge') %>% pull(xlabels)) + ylab("Partial dependence effect") + 
  theme(legend.position="bottom", legend.direction="horizontal", 
        plot.title = element_text(size=18, margin=margin(0,0,50,0)),
        axis.title=element_text(size=12), plot.title.position = "plot",
        plot.subtitle=element_text(size=18, color="black")) + 
...
NC_PDP_plot_ageph_ModelComp_NOR <- rbind(
  ...
  theme_bw() + 
  guides(color = guide_legend(nrow = 1, byrow = TRUE)) + 
  xlab(vars_with_label_NOR %>% filter(Variable == 'Young') %>% pull(xlabels)) + ylab("Partial dependence effect") + 
  theme(legend.position="bottom", legend.direction="horizontal", 
        plot.title = element_text(size=18, margin=margin(0,0,50,0)),
        axis.title=element_text(size=12), plot.title.position = "plot",
        plot.subtitle=element_text(size=18, color="black")) + 
...
NC_PDP_plot_BM_ModelComp_BE <- rbind(
  ...
  theme_bw() + 
  guides(color = guide_legend(nrow = 1, byrow = TRUE)) + 
  xlab(vars_with_label_BE %>% filter(Variable == 'bm') %>% pull(xlabels)) + ylab("Partial dependence effect") + 
  theme(legend.position="bottom", legend.direction="horizontal", 
        plot.title = element_text(size=18, margin=margin(0,0,50,0)),
        axis.title=element_text(size=12), plot.title.position = "plot",
        plot.subtitle=element_text(size=18, color="black")) + 
...
NC_PDP_plot_BM_ModelComp_FR <- rbind(
  ...
  theme_bw() + 
  guides(color = guide_legend(nrow = 1, byrow = TRUE)) + 
  xlab(vars_with_label_FR %>% filter(Variable == 'BonusMalus') %>% pull(xlabels)) + ylab("Partial dependence effect") + 
  theme(legend.position="bottom", legend.direction="horizontal", 
        plot.title = element_text(size=18, margin=margin(0,0,50,0)),
        axis.title=element_text(size=12), plot.title.position = "plot",
        plot.subtitle=element_text(size=18, color="black")) + 
...
EMP_NC <- data_BE_PC %>% select(POSTCODE = postcode, NC = nclaims, expo = expo) %>% 
  group_by(POSTCODE) %>% 
  summarise(NC = sum(NC)/sum(expo))
...
NC_PDP_plot_spatial_GBM_BE <- tm_shape(NC_PDP_spatial_GBM_BE) + 
  ...
  tm_fill(col = "NC", title = "Partial dependence effect", 
...
load('NC_CANN_GBM_flex_SURR_allFolds_FR_30082023version')
...
NC_SURR_coefficients_FR <- summary(NC_CANN_GBM_flex_SURR_allFolds_FR_30082023version[[1]]$best_surr)$coefficients %>% 
...
NC_SURR_coefficients_FR_singleeffect <- NC_SURR_coefficients_FR %>% slice(1:53)
all_vars_singeleffect <- (NC_CANN_GBM_flex_SURR_allFolds_FR_30082023version[[1]]$slct_feat %>% names)[1:8]
...
policyholder_selection <- tibble(
  ...
  Density_ = NC_SURR_coefficients_FR_singleeffect %>% filter(grepl('Density',rowname)) %>% arrange(coef) %>% 
    slice(1,8,12) %>% mutate(rowname = str_replace(rowname,'Density_','')) %>% pull(rowname),
  Region_ = NC_SURR_coefficients_FR_singleeffect %>% filter(grepl('Region',rowname)) %>% arrange(coef) %>% 
    slice(1,3,6) %>% mutate(rowname = str_replace(rowname,'Region_','')) %>% pull(rowname)
) 

# Manually change some to reference level
policyholder_selection[2,'VehAge_'] <- '[1, 2]'
policyholder_selection[3,'VehAge_'] <- '[1, 2]'
policyholder_selection[1,'BonusMalus_'] <- '[50, 54]'
policyholder_selection[1,'VehGas_'] <- '{Regular}'
...
policyholder_selection_pred_forCANN <- policyholder_selection_pred %>% 
  mutate(
    ...
    VehAge = c(3,2,1),
    DrivAge = c(2,4,7),
    BonusMalus = c(50,70,190),
    VehBrand = c('B12','B5','B11'),
    VehGas = c('Regular','Regular','Diesel'),
    Density = c(1,6.5,10),
    Region = c('Midi-Pyrenees','Basse-Normandie','Corse'),
    Area = c('A','B','C')
  )

pol <- 1
NC_CANN_GBM_flex_SURR_allFolds_FR_30082023version[[1]]$best_surr$data %>% 
  #filter(VehPower == policyholder_selection_pred_forCANN[pol,]$VehPower) %>% 
  #filter(VehAge == policyholder_selection_pred_forCANN[pol,]$VehAge) %>% 
  filter(DrivAge == policyholder_selection_pred_forCANN[pol,]$DrivAge) %>% 
  filter(BonusMalus == policyholder_selection_pred_forCANN[pol,]$BonusMalus) %>% 
  #filter(VehBrand == policyholder_selection_pred_forCANN[pol,]$VehBrand) %>% 
  #filter(VehGas == policyholder_selection_pred_forCANN[pol,]$VehGas) %>% 
  #filter(Density == policyholder_selection_pred_forCANN[pol,]$Density) %>% 
  #filter(Region == policyholder_selection_pred_forCANN[pol,]$Region) %>% 
  distinct(DrivAge_BonusMalus_)

# Add interaction values to the policyholders, look at missing value in the coefficients to find the reference level
policyholder_selection <- policyholder_selection %>% mutate(expo = 1) %>% 
  mutate(VehBrand_Region_ = c('7','4','5')) %>% 
  mutate(Density_Region_ = c('6','4','3')) %>% 
  mutate(VehPower_Region_ = c('3','4','4')) %>% 
  mutate(VehPower_VehBrand_ = c('3','3','2')) %>% 
  mutate(DrivAge_Region_ = c('6','5','1')) %>% 
  mutate(BonusMalus_Region_ = c('10','10','8')) %>% 
  mutate(VehBrand_Density_ = c('4','3','5')) %>% 
  mutate(BonusMalus_VehBrand_ = c('11','10','9')) %>% 
  mutate(DrivAge_BonusMalus_ = c('8','4','7'))
...
policyholder_selection_pred <- bind_cols(policyholder_selection,
                                         pred = predict(NC_CANN_GBM_flex_SURR_allFolds_FR_30082023version[[1]]$best_surr, 
...
coef_plots <- lapply(c(1:3), function(risk){
  NC_CANN_GBM_flex_SURR_allFolds_FR_30082023version[[1]]$best_surr %>% 
...
policyholder_selection_pred_forCANN <- policyholder_selection_pred_forCANN %>% 
...
french_regions <- tribble(
  ...
  "Rhone-Alpes", "Rh\xf4ne-Alpes", 'Group_6',
  "Picardie", "Picardie", 'Group_6',                
  "Aquitaine", "Aquitaine", 'Group_5',                
  "Nord-Pas-de-Calais", "Nord-Pas-de-Calais", 'Group_3',      
  "Languedoc-Roussillon", "Languedoc-Roussillon", 'Group_2',     
  "Pays-de-la-Loire", "Pays de la Loire", 'Group_5',           
  "Provence-Alpes-Cotes-D'Azur", "Provence-Alpes-C\xf4te d'Azur", 'Group_5',
  "Ile-de-France",  "\xcele-de-France", 'Group_7',            
  "Centre", "Centre", 'Group_4',
  "Corse",  "Corse", 'Group_7',                   
  "Auvergne", "Auvergne", 'Group_2', 
  "Poitou-Charentes", "Poitou-Charentes", 'Group_5',         
  "Bourgogne", "Bourgogne", 'Group_3',                  
  "Bretagne", "Bretagne", 'Group_4',               
  "Midi-Pyrenees", "Midi-Pyr\xe9n\xe9es", 'Group_1',             
  "Alsace", "Alsace", 'Group_5',                     
  "Basse-Normandie", "Basse-Normandie", 'Group_3',           
  "Champagne-Ardenne", "Champagne-Ardenne", 'Group_5',        
  "Franche-Comte", "Franche-Comt\xe9", 'Group_2',             
  "Limousin", "Limousin", 'Group_6',                  
  "Haute-Normandie", "Haute-Normandie",'Group_3'
)
...
# Look at grouping in the surrogate
NC_SURR_coefficients_FR_singleeffect %>% filter(grepl('Region',rowname)) %>% arrange(coef)

# Add surrogate groups per French region
french_shape_sf <- french_shape_sf %>% bind_cols(Group = c(5,5,2,7,3,3,4,4,5,7,2,3,2,6,1,3,5,6,5,5,6)) %>% 
...
NC_PDP_spatial_FR_plot <- tm_shape(NC_PDP_spatial_FR, bbox = bbox_new) + 
  tm_borders(col = "black") + 
  tm_fill(col = "y", title = "Partial dependence \neffect", 
          textNA = "No Policyholders", 
          #style = "cont", 
          palette = "Accent", 
          colorNA = "white") +
...
breaks <- get_splits(NC_CANN_GBM_flex_SURR_allFolds_FR_30082023version[[1]],'BonusMalus_') %>% 
...
NC_PDP_BonusMalus_FR_Surr <- FR_PDP_BonusMalus_withGroups %>% mutate(Group = paste0("Group_",gr)) %>% 
  ...
  ylab("Partial dependence effect") + 
...
get_splits(NC_CANN_GBM_flex_SURR_allFolds_FR_30082023version[[1]],'DrivAge_')

NC_PDP_DrivAge_FR_Surr <- FR_PDP_DrivAge %>% 
  arrange(x) %>% 
  bind_cols(Group = c("Group_1","Group_2","Group_3","Group_4","Group_5","Group_6","Group_7")) %>% 
