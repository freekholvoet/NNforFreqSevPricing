...
FR_tuning_GBM_sev <- tune_GBM_sev(data_FR, feat_FR, tuning_grid, data_folds = 1:6)
...
get_opt_param <- function(tuning_results){
  summariser_per_testfold <- tuning_results %>% 
    ...
    gather(- T, -d, key = 'Test_Fold', value = 'Cross_Val_Error') %>% 
    mutate(Test_Fold = str_remove(Test_Fold, "Var_Errors.")) %>% 
    mutate(Test_Fold = str_remove(Test_Fold, "_mean")) %>% 
    mutate(Test_Fold = as.numeric(Test_Fold))
...
fit_sev_GBM <- function(data, data_feat, parameters, data_folds){
  lapply(data_folds, function(fold){
    ...
    return(list(data_GBM, 
                dev_gamma(data_GBM %>% filter(fold_nr==fold) %>% pull(average), 
                          data_GBM %>% filter(fold_nr==fold) %>% pull(prediction), 
...
oos_sev_GBM_AUS <- fit_sev_GBM(data_AUS %>% filter(nclaims > 0) %>% filter(!is.na(average)), feat_AUS, opt_tuning_sev_AUS, 1:6)

oos_freq_GBM_FR <- fit_freq_GBM(data_FR, feat_FR, opt_tuning_freq_FR, 1:6)
oos_sev_GBM_FR <- fit_sev_GBM(data_FR %>% filter(nclaims > 0), feat_FR, opt_tuning_sev_FR, 1:6)

oos_freq_GBM_NOR <- fit_freq_GBM(data_NOR, feat_NOR, opt_tuning_freq_NOR, 1:6)
oos_sev_GBM_NOR <- fit_sev_GBM(data_NOR %>% filter(nclaims > 0) %>% filter(!is.na(average)), feat_NOR, opt_tuning_sev_NOR, 1:6)
...
# Check on French balance

load('oos_freq_GBM_FR')
load('oos_sev_GBM_FR')

sapply(1:6, function(f) oos_freq_GBM_FR[[f]][[1]] %>% filter(fold_nr == f) %>% summarise(pf = sum(prediction), of = sum(nclaims), ratio = pf/of))
sapply(1:6, function(f) oos_sev_GBM_FR[[f]][[1]] %>% filter(fold_nr == f) %>% summarise(ps = sum(prediction), os = sum(average), ratio = ps/os))

oos_preds <- lapply(1:6, function(fold){
  d <- data_FR %>% filter(fold_nr == fold)
  d_freq <- oos_freq_GBM_FR[[fold]][[3]] %>% predict_model.gbm(., d) * d$expo
  d_sev <- oos_sev_GBM_FR[[fold]][[3]] %>% predict_model(newdata = d)
  tibble(d,gbm_pred_poiss = d_freq, gbm_pred_gamma = d_sev)
}) %>% do.call(rbind,.)

data_FR_GBMtariff <- oos_preds %>% 
  mutate(gbm_predlosses = gbm_pred_poiss*gbm_pred_gamma) %>% 
  mutate(amount = nclaims*average) %>% 
  mutate_at(vars(amount), ~replace(., is.na(.), 0))

data_FR_GBMtariff %>% 
  summarise(Oclaims = sum(nclaims), Pclaims = sum(gbm_pred_poiss), Ratio_freq = Pclaims / Oclaims,
            Oamount = sum(average,na.rm = T), Pamount = sum(gbm_pred_gamma), Ratio_sev = Pamount / Oamount,
            Oloss = sum(amount), Ploss = sum(gbm_predlosses), Ratio_loss = Ploss / Oloss)
