...
fit_Intr_sev_FR <- lapply(1:6, function(fold){
  
  FR_interactVar <- paste(intersect(feat_FR[!!as.matrix(opt_fit_GAM_sev_FR[[fold]] %>% select(!BIC_value))],setdiff(feat_FR,cat_FR)),collapse = ',')
...
oos_binned_sev_GLM_FR <- add_oos_predictions(data_FR %>% filter(nclaims > 0), binned_sev_FR)
...
FR_pred_NC <- lapply(1:6, function(fold){
  
  model_set <- binned_freq_FR[[fold]]
  data_f <- data_FR %>% filter(fold_nr == fold) %>% filter(!(nclaims>0 & is.na(average)))
  ...
  # Predicting with GLM
  bind_cols(binned_data, prediction = predict(model_set[[2]], binned_data, type = 'response'))
  
}) %>% do.call(rbind,.)

FR_pred_CA <- lapply(1:6, function(fold){
  
  model_set <- binned_sev_FR[[fold]]
  data_f <- data_FR %>% filter(fold_nr == fold) %>% filter(!(nclaims>0 & is.na(average)))
...
data_FR_GLMtariff <- left_join(FR_pred_NC %>% select(id,nclaims,glm_poiss = prediction,fold_nr), 
                               FR_pred_CA %>% select(id,average,glm_gamma = prediction)) %>% 
  mutate(glm_predlosses = glm_poiss*glm_gamma) %>% 
  mutate(amount = nclaims*average) %>% 
  mutate_at(vars(amount), ~replace(., is.na(.), 0))

data_FR_GLMtariff %>% 
  summarise(Oclaims = sum(nclaims), Pclaims = sum(glm_poiss), Ratio_freq = Pclaims / Oclaims,
            Oamount = sum(average,na.rm = T), Pamount = sum(glm_gamma), Ratio_sev = Pamount / Oamount,
            Oloss = sum(amount), Ploss = sum(glm_predlosses), Ratio_loss = Ploss / Oloss)
